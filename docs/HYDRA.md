...
This is Blastline’s central distinction from a present-day dependency lookup. It does not merely ask which repositories depend on a package now. It reconstructs which repositories resolved the compromised version during the requested exposure window, preserves the relationship path through HydraDB, and compares that historical set with the current set. A candidate path is evidence to verify, not a security answer to trust blindly.
